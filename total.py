...
class AutoClickerMacro:
    def __init__(self, callbacks):
        self.c = callbacks # UI 콜백 함수 딕셔너리
        self.is_running = False
        self.is_paused = False
        self.macro_thread = None
        self.stop_event = threading.Event()
        self.pause_event = threading.Event()
        self.pause_event.set() # 기본은 실행 상태
    ...
    def stop(self):
        ...
        self.is_running = False
        self.stop_event.set()
        self.pause_event.set() # 중지 시 일시정지 해제
...
class RecordingMacroTab(ttk.Frame):
    def __init__(self, parent, app, config=None):
        ...
        self.is_dirty = False # 변경사항 여부 플래그
        self._create_widgets()
    ...
    def save_chain(self, save_as=False):
        ...
        if save_as or not file_path:
            file_path = filedialog.asksaveasfilename(
                title="매크로 체인 저장", defaultextension=".pchain",
        ...
        for item_id in self.macro_tree.get_children():
            ...
            if item_data['type'] in [MacroType.FILE.value, MacroType.IMAGE_WAIT.value]:
                try:
                    data_path = os.path.relpath(data_path, chain_dir)
                except ValueError:
                    pass

            chain_item = {
                'type': item_data['type'],
                'data': data_path if item_data['type'] != MacroType.MEMORY.value else item_data['data'],
    ...
    def update_ui_state(self, recording=None, playing=None, paused=None):
        try:
            ...
            if playing is not None:
                ...
                for btn in [self.add_file_button, self.add_wait_image_button, self.remove_button, self.up_button, self.down_button, self.clear_button, self.save_chain_button, self.load_chain_button, self.save_as_chain_button]:
    ...
    def on_tree_double_click(self, event):
        ...
        elif column == "#1" and item_data['type'] != MacroType.FILE.value:
            self.edit_tree_cell(selected_item, column)
...
class MainApp:
    ...
    def save_config(self):
        try:
            config_data = {
                ...
                "image_tab": {
                    "last_folder": self.image_tab.image_folder_path.get()
                },
                "record_tab": {
    ...
    def _on_key_press(self, key):
        try:
            ...
            if current_tab_index == 0: # 매크로 체인 탭
